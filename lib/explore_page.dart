...
import 'package:quake_vision/earth_seismic_events.dart';
import 'package:quake_vision/stations_data.dart';
...
class ExplorePageState extends State<ExplorePage>
  ...
  initState() {
    ...
    _controller = FlutterEarthGlobeController(
        ...
        surface: Image.asset('assets/2k_moon.jpg').image);
    points = getMoonQuakeStations(context);
  ...
  void _onEventSelected(String? newValue) {
    setState(() {
      ...
      final selectedEvent = seismicEvents.firstWhere(
          (event) =>
              "${event['sensor']} ${event['date']} ${event['time']}" ==
              newValue,
          orElse: () => {});
          ...
          .map((texture) => Padding(
                ...
                child: Card(
                  color: _selectedSurface == texture
                      ? Colors.grey[800]
                      : const Color.fromARGB(255, 2, 10, 28),
                  child: InkWell(
                    onTap: () {
                      ...
                      setState(() {
                        ...
                        if (texture.contains('mars')) {
                          points = getMarsQuakeStations(context);
                          seismicEvents = marsSeismicEvents;
                        } else if (texture.contains('moon')) {
                          points = getMoonQuakeStations(context);
                          seismicEvents = moonSeismicEvents;
                        } else {
                          points = getEarthQuakeStations(context);
                          seismicEvents = earthSeismicEvents;
                        }
              ...
              child: Container(
                ...
                child: Column(
                    ...
                    children: [
                      ...
                      if (_selectedEventData != null) ...[
                        Text(
                          'Location: ${_selectedEventData!['location'] ?? 'N/A'}',
                          style: const TextStyle(color: Colors.white),
                        ),
                        Text(
                          'Sensor: ${_selectedEventData!['sensor'] ?? 'N/A'}',
                          style: const TextStyle(color: Colors.white),
                        ),
                        Text(
                          'Date of detection: ${_selectedEventData!['date'] ?? 'N/A'}',
                          style: const TextStyle(color: Colors.white),
                        ),
                        Text(
                          'Time of detection: ${_selectedEventData!['time'] ?? 'N/A'}',
                          style: const TextStyle(color: Colors.white),
                        ),
                        Text(
                          'Magnitude: ${_selectedEventData!['magnitude'] ?? 'N/A'}',
                          style: const TextStyle(color: Colors.white),
                        ),
                        Text(
                          'Depth: ${_selectedEventData!['depth'] ?? 'N/A'}',
                          style: const TextStyle(color: Colors.white),
                        ),
                        Text(
                          'Event Type: ${_selectedEventData!['eventType'] ?? 'N/A'}',
                          style: const TextStyle(color: Colors.white),
                        ),
                        Text(
                          'Trigger On (s): ${_selectedEventData!['Trigger On (s)'] ?? 'N/A'}',
                          style: const TextStyle(color: Colors.white),
                        ),
                        Text(
                          'Trigger Off (s): ${_selectedEventData!['Trigger Off (s)'] ?? 'N/A'}',
                          style: const TextStyle(color: Colors.white),
                        ),
                      ]
                    ],
            ...
            Positioned(
              ...
              child: Container(
                ...
                child: Column(
                  ...
                  children: [
                    Text(
                      !_selectedSurface!.contains('earth')
